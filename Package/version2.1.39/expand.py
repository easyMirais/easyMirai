...
class expandPoke:
    ...
    def _context(self, name: str, to: str):
        ...
        data = po.post(
            self._url + _config["send"]["friend"][to], data=json.dumps(data))
...
class expandNudge:
    ...
    def _context(self, name: str, by: str, to: str, subject: int):
        ...
        data = po.post(
            self._url + _config["send"][by][to], data=json.dumps(data))
        if data.status_code == 200:
            data = json.loads(data.text)
            if data["code"] == 0:
                self._c.log("[Notice]：捏捏发送成功",
                            "详细：" + str(self._target) + "(" + name +
                            " " + str(subject) + ") <- '捏捏'",
                            style="#a4ff8f")
            else:
...
class expandTemp:
    ...
    def plain(self, context: str):
        ...
        print(data)
        data = po.post(
            self._url + _config["send"]["temp"]["sendTempMessage"], data=json.dumps(data))
        if data.status_code == 200:
            data = json.loads(data.text)
            if data["code"] == 0:
                self._c.log("[Notice]：文本发送成功",
                            "详细：" + str(self._target) + "(Temp " +
                            str(self._gid) + ") <- '" + str(context) + "'",
                            style="#a4ff8f")
            else:
        ...
        if data.status_code == 200:
            ...
            if data["code"] == 0:
                self._c.log("[Notice]：表情发送成功",
                            "详细：" + str(self._target) + "(Group " +
                            str(self._gid) + ") <- '" + str(faceId) + "'",
                            style="#a4ff8f")
            else:
...
class expandGetMessage:
    ...
    def _request(self, message, to: str, params: dict, isSlice: bool = False):
    ...
    def count(self, isSlice: bool = False):
        data = {
            "sessionKey": self._session
        }
        return self._request("count", "count", data, isSlice)
...
class expandSetGroupConfig:
    ...
    def name(self, name: str):
        ...
        data = po.post(self._url + _config["get"]
                       ["groupConfig"], data=json.dumps(data))
        if data.status_code == 200:
            data = json.loads(data.text)
            if data["code"] == 0:
...
class expandEventNewFriend:
    ...
    def _request(self, code: int, eventId, message):
        ...
        data = po.post(
            self._url + _config["event"]["newFriend"], data=json.dumps(data))
        if data.status_code == 200:
            data = json.loads(data.text)
            if data["code"] == 0:
...
class expandEventBotJoinGroup:
    ...
    def _request(self, code: int, message):
        ...
        data = po.get(self._url + _config["event"]
                      ["newFriend"], data=json.dumps(data))
        if data.status_code == 200:
            data = json.loads(data.text)
            if data["code"] == 0:
                self._c.log("[Notice]：用户入群申请事件处理成功",
...
class expandActionGroup:
    ...
    def unmute(self, target: int):
        ...
        data = po.post(
            self._url + _config["action"]["unmute"], data=json.dumps(data))
        if data.status_code == 200:
            data = json.loads(data.text)
    ...
    def muteAll(self):
        ...
        data = po.post(
            self._url + _config["action"]["muteAll"], data=json.dumps(data))
        if data.status_code == 200:
            data = json.loads(data.text)
    ...
    def unMuteAll(self):
        ...
        data = po.post(
            self._url + _config["action"]["unmuteAll"], data=json.dumps(data))
        if data.status_code == 200:
            data = json.loads(data.text)
    ...
    def kick(self, target: int):
        ...
        data = po.post(
            self._url + _config["action"]["kick"], data=json.dumps(data))
        if data.status_code == 200:
            data = json.loads(data.text)
            if data["code"] == 0:
    ...
    def quit(self):
        ...
        data = po.post(
            self._url + _config["action"]["quit"], data=json.dumps(data))
        if data.status_code == 200:
            data = json.loads(data.text)
            if data["code"] == 0:
...
class expandActionFriend:
    ...
    def deleteFriend(self, target: int):
        ...
        data = po.post(
            self._url + _config["action"]["deleteFriend"], data=json.dumps(data))
        if data.status_code == 200:
            data = json.loads(data.text)
            if data["code"] == 0:
...
class expandActionGroupMute:
    ...
    def _request(self, time: int):
        ...
        data = po.post(
            self._url + _config["action"]["mute"], data=json.dumps(data))
        if data.status_code == 200:
            data = json.loads(data.text)
            if data["code"] == 0:
                self._c.log("[Notice]：禁言成功",
                            "详细：" + str(self._target) + "(Group) <- '" +
                            str(time) + " s " + str(self._memberId) + "'",
                            style="#a4ff8f")
            else:
...
def _uploadImage(path: str, session, name, url: str):
    ...
    data = po.request(method="POST", url=url + "/uploadImage",
                      data=message, files=onfiles)
    if data.status_code == 200:
        data = json.loads(data.text)
        if "code" not in data:
            return data["url"]
